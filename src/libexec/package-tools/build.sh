...
step_run() {
    step $1; shift
    if [[ $1 == "--exit" ]]; then
        STEP_EXIT=true; shift
    fi
    $@
    if [[ $STEP_EXIT == true ]]; then
        step_done $? --exit
    else
        step_done $?
    fi
...
if [[ $BUILD_DEB == true ]]; then
    ...
    eval 'mkdir -p package/deb &&\
    wget "https://github.com/azukiapp/libnss-resolver/releases/download/v${LIBNSS_RESOLVER_VERSION}/ubuntu12-libnss-resolver_${LIBNSS_RESOLVER_VERSION}_amd64.deb" -O "package/deb/precise-libnss-resolver_${LIBNSS_RESOLVER_VERSION}_amd64.deb" && \
    wget "https://github.com/azukiapp/libnss-resolver/releases/download/v${LIBNSS_RESOLVER_VERSION}/ubuntu14-libnss-resolver_${LIBNSS_RESOLVER_VERSION}_amd64.deb" -O "package/deb/trusty-libnss-resolver_${LIBNSS_RESOLVER_VERSION}_amd64.deb"' && \

    step_run "Creating deb packages" make package_deb && \
...
if [[ $BUILD_RPM == true ]]; then
    ...
    step_run "Downloading libnss-resolver" \
    eval 'quiet mkdir -p package/deb &&\
    quiet wget "https://github.com/azukiapp/libnss-resolver/releases/download/v${LIBNSS_RESOLVER_VERSION}/fedora20-libnss-resolver-${LIBNSS_RESOLVER_VERSION}-1.x86_64.rpm" -O "package/rpm/fedora20-libnss-resolver-${LIBNSS_RESOLVER_VERSION}-1.x86_64.rpm"' && \

    step_run "Creating rpm packages" make package_rpm LINUX_CLEAN= && \
