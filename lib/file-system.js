...
var createProjectDirectory = function(fs, dirPath, mode) {
  var deferred = Q.defer();
  fs.isDirectory(dirPath).then(function(isDir) {
    if(isDir) {
      deferred.reject('dir-exists');
      return new Error(error);
    } else {
      fs.makeDirectory(dirPath).then(function() {
        deferred.resolve(dirPath);
      }, function(error) {
        deferred.reject('file-exists');
      });
    }
  });
...
var create = function(fs) {
  return {
    ...
    createProjectDirectory: createProjectDirectory.bind(null, fs || Qfs),
