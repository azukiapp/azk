...
AzkDocker.prototype.findContainer = function(id) {
  var self = this;
  return self.getContainer(id).inspect()
    .then(function(data) {
...
AzkDocker.prototype.run = function(image, cmd, opts) {
  ...
  var name = opts.name || generate_name(opts.ns || "run");
  opts.env = opts.env || {};
  ...
  .then(function() {
    done.notify({ type: "removing", id: container.id });
...
// log
azk.debug("docker")(azk.t("docker.connect", cst.DOCKER_HOST));
