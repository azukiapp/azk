...
var _ = azk._;
var l_debug = azk.debug("azk:proxy");
...
Proxy.remove = function(hosts, backend) {
  _.each(_.isString(hosts) ? [hosts] : hosts, function(host) {
    ...
    data[host] = _.filter(entries, function(entry) {
      if (entry == backend) {
        l_debug("proxy.removing_backend", { hostname: host, backend: backend });
        return false;
      }
      return true;
    });
...
Proxy.register = function(hosts, backend) {
  _.each(_.isString(hosts) ? [hosts] : hosts, function(host) {
    ...
    if (!_.include(entries, backend)) {
      l_debug("proxy.adding_backend", { hostname: host, backend: backend });
...
Proxy.start = function(port, debug) {
  ...
  var requestHandler = function(req, res) {
    ...
    if (address) {
      debug("proxy.request", host, address);
      try {
      ...
      debug("proxy.not_configured", host);
      Proxy.responseStatic(400, res);
    }
  }

  port = port || azk.cst.DAEMON_PROXY_PORT;
  debug("proxy.started", port);
