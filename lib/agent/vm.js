...
vm.delete = function(name) {
  return Q.async(function* () {
    ...
    if (info.name == name) {
      // Removing disk
      //if (typeof(info['IDE-0-0']) != 'undefined') {
        //yield exec("storagectl", name, "--name", "IDE", "--remove");
        //yield exec("closemedium", "disk", info['IDE-0-0']);
      //}
...
vm.wait = function(port) {
  return azk.utils.ping({ host: 'localhost', port: port })
  .then(function(result) {
    if (!result) {
      return vm.wait(port);
    }
...
function ssh_run(ip, port, cmd) {
  ...
  c.connect({
    ...
    //password: "tcuser",
    password: "live",
    //privateKey: fs.readFileSync(azk.cst.VM_KEY),
  });
