...
($traceurRuntime.createClass)(Cmd, {
  action: function(opts, extras) {
    ...
    var progress = (function(event) {
      if (pull_progress(event)) {
        if (event.type == "started" && event.context == "container_run") {
...
function init(cli) {
  (new Cmd('shell', cli)).addOption(['-T']).addOption(['-t']).addOption(['--rm', '-r'], {default: true}).addOption(['--system', '-s'], {type: String}).addOption(['--image', '-i'], {type: String}).addOption(['--command', '-c'], {type: String}).addOption(['--shell'], {type: String}).addOption(['--cwd', '-C'], {type: String}).addOption(['--mount', '-m'], {
    type: String,
    acc: true,
    default: []
  }).addOption(['--env', '-e'], {
