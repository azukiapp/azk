...
function init(cli) {
  (new Cmd('agent {action}', cli)).setOptions('action', {options: ['start', 'status', 'stop']}).addOption(['--daemon', '-d'], {default: false}).addOption(['--reload-vm', '-d'], {default: true}).addOption(['--check-install'], {default: false});
}
module.exports = {
  get init() {
    return init;
