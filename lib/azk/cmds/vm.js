module.exports = function() {
  ...
  ($traceurRuntime.createClass)(VmCmd, {
          ...
          while (true)
            switch ($ctx.state) {
              ...
              case 2:
                ...
                break;
              case 4:
                promise = this[("action_" + action)](vm_info, opts);
    ...
    action_ssh: function(vm_info, opts) {
          ...
          while (true)
            switch ($ctx.state) {
              ...
              case 2:
                ...
                break;
              case 4:
                ssh_url = (config('agent:vm:user') + "@" + configs['agent:vm:ip']);
                ssh_opts = "StrictHostKeyChecking=no -o LogLevel=quiet -o UserKnownHostsFile=/dev/null";
                args = opts.__leftover.join("\" \"");
                script = ("ssh -i " + config('agent:vm:ssh_key') + " -o " + ssh_opts + " " + ssh_url + " \"" + args + "\"");
                this.info(script);
                $ctx.state = 8;
                break;
              case 8:
                $ctx.returnValue = this.execSh(script);
                $ctx.state = -2;
                break;
              default:
                return $ctx.end();
  ...
  function init(cli) {
    if (config('agent:requires_vm')) {
      (new VmCmd('vm {*action}', cli)).setOptions('action', {options: ['ssh', 'install', 'installed', 'start', 'status', 'stop', 'remove', 'reload']});
    }
  }
  return {
    get init() {
