  ...
  ($traceurRuntime.createClass)(Configure, ($__2 = {}, Object.defineProperty($__2, "run", {
    ...
    value: function() {
      if (config('agent:requires_vm')) {
        ...
        return Q.all([this._checkDockerSocket(socket), this._checkAndConfigureNetwork(false), this._loadDnsServers(), this._checkPorts('agent:balancer:port', 'balancer', 'AZK_BALANCER_PORT'), this._checkPorts('agent:dns:port', 'dns', 'AZK_DNS_PORT')]).fail((function(err) {
          if (!err instanceof DependencyError)
            err = new DependencyError('docker_access', {socket: socket});
          throw err;
        }));
      }
    },
    ...
    enumerable: true,
    writable: true
  }), Object.defineProperty($__2, "_checkPorts", {
    value: function(confKey, service, env) {
      var port = config(confKey);
      return net.checkPort(port, this.docker_ip).then((function(avaibly) {
        if (!avaibly)
