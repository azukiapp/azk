...
var Unfsd = {
  ...
  mount: function(vm_name) {
    ...
    return Tools.defer_status("unsfd", (function(_resolve, _reject, change_status) {
      ...
      var cmd = [("[ -d \"" + point_nfs + "\" ] || { mkdir -p " + point_nfs + "; }"), "{ " + check_nfs + " || " + mount + "; }", ("[ -d \"" + point + "\" ] || { mkdir -p " + point + "; }"), "{ " + check + " || " + bindfs + "; }", "{ " + check_nfs + " && " + check + "; }"].join("; ");
      var stderr = "";
  ...
  _checkConfig: function(vm_ip) {
    var file = config('paths:unfsd_file');
    fs.writeFileSync(file, ["# All", "/ " + net.calculateNetIp(vm_ip) + "(rw)"].join("\n"));
    return file;
  }
};
;
