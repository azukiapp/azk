...
var docker_host = process.env.AZK_DOCKER_HOST || process.env.DOCKER_HOST || (requires_vm ? "http://azk-agent:4243" : "unix:///var/run/docker.sock");
...
var options = merge({
  ...
  test: {
    ...
    agent: {vm: {data_disk: path.join(data_path, "vm", "azk-agent-spec.vmdk")}}
  }
});
function env() {
